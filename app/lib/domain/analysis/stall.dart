/// A2.4 — the stall detector (design 09 §9.4).
///
/// The classic barbecue stall: evaporative cooling balances heat input and a
/// large cut sits at 150–170 °F for hours. Worth surfacing because it
/// *prevents* action: "the stall is normal, do not raise the pit
/// temperature".
///
///   enter:  role == food ∧ |slope| < 2 °F/hr sustained ≥ 30 min
///           ∧ 140 °F ≤ temp ≤ 180 °F
///   exit:   slope > 4 °F/hr sustained ≥ 15 min
///
/// The sustain windows are the hysteresis — a probe hovering at either
/// threshold must not oscillate.
library;
...
class StallDetector {
  ...
  bool add(int t, double? f) {
    ...
    _series.add((t: t, f: f));
    // Keep ~45 min — enough for the 10-min slope plus the sustain windows.
