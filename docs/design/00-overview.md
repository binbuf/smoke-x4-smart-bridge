...
| Display is a static 7-line status dump   | Underuses a 128×64 OLED and the button                                                                                                 | Paged info views + sparkline on the glass; control lives in the app, D15 ([07](07-display-and-controls.md)) |
...
- Two network modes — **Hosted AP** and **Joined STA** — selectable at boot, from BLE, or from the
  HTTP API (not from the button: see D15)
...
- ~~MQTT / Home Assistant publishing~~ — **added back after v1 (A16, see [05 §5.7](05-connectivity-and-provisioning.md)).**
  D2 shaped the event bus so a publisher module dropped in without refactoring; it now has: `app_mqtt`
  (core + glue), plaintext `mqtt://` with Home Assistant MQTT Discovery, gated on STA-up and opt-in,
  configured from the app. TLS remains out (its own heap conversation).
...
| **D2**  | ~~No MQTT / Home Assistant in v1~~ · **reversed (A16)**         | Kept MQTT out of v1 to remove a large config surface; the event bus was shaped so it dropped back in as **one module** (`app_mqtt`) with no refactoring — exactly as promised. Post-v1 it publishes plaintext `mqtt://` with HA MQTT Discovery, gated on STA-up, opt-in, configured from the app. TLS/cert-upload stays out (its own heap conversation). See [05 §5.7](05-connectivity-and-provisioning.md). |
| **D3**  | **One button, gesture-driven** — *superseded by D15*            | The board exposes exactly one usable button (PRG on GPIO0). No soldering, no enclosure work. Originally tap / double-tap / hold-2s / hold-10s / hold-at-boot. **The gesture ladder was removed in v1.1 (D15):** tap cycles info views, hold powers off, and nothing else. Layout in [07 §7.4](07-display-and-controls.md).                                            |
...
| **D13** | **Fallback web UI: settled in M6/V4.2 — NOT in v1.0.0; partition kept** | Whether the device serves its own browser UI was a post-MVP call, decided on evidence from real use as the milestone plan specified. **Decided 2026-07-23: no, and the partition is not reclaimed either.** Every case [06 §6.4](06-device-api.md) makes for it is now covered — setup/recovery by the T5 browser installer and BLE onboarding, a captive target by the built-in page + flash-resident probe answers (verified on the board in M2), and toolchain-free debugging by `curl` against the OpenAPI spec plus `/debug/packets`, `/novelty`, `/coredump` and `/tasks`. The 512 KB `www` partition stays declared and unformatted and `ops_www_available()` keeps returning `false`: reclaiming it is a repartition that erases every stored cook (D8's reasoning), and 2,432 KB is already 54 days of history, so nobody is short. Revisit in v1.1 only if a concrete need appears; the reservation keeps that free. |
| **D14** | **Default display unit is °F**                                  | Storage is canonical °F either way ([04 §4.2](04-storage-and-history.md)); this is only the initial display preference, switchable in the app (D15 moved every control off the device).                                                                                                                                                                                        |
| **D15** | **The bridge is a passthrough: all control lives in the app** | The on-device action set kept growing — per-page context actions, then a 2 s/5 s/10 s hold ladder, then a menu — and each version was worse than the last. One button cannot carry eight verbs, and asking a user to release inside a time-window is not an interface. Units, sessions, marks, pairing, network mode, battery saver, factory reset and alarm silence all move to the app, which already reaches the device over HTTP and BLE and is the only surface with room for them. The button keeps exactly two jobs — **tap cycles info views, hold powers off** — and the OLED becomes a display. Alarms are shown on the glass but silenced on the Smoke X receiver or in the app: the bridge reports, it does not resolve. Layout in [07 §7.4](07-display-and-controls.md); the remote surface in [06](06-device-api.md) and [05](05-connectivity-and-provisioning.md). |
