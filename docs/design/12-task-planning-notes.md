...
| **F8 + A14** | AP-mode routing on Android is the classic "works on my phone" trap | Plan both mitigations (captive shim *and* `bindProcessToNetwork`) as one unit of work; neither alone is sufficient |
...
Deliberately no time estimates here — velocity is unknown. These are *relative* risk flags.
