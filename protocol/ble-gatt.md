...
- Temperatures are **`i16`, tenths of °F**, with the named sentinels
  `TEMP_DETACHED = -32768` (`INT16_MIN`, probe detached — must surface as _null_, never 0) and
...
one is assigned _(defined by this document — design 05 is silent)_.
...
#### 5.4.1 `auth` values _(defined by this document — `records.yaml` declares a plain u8; values follow ESP-IDF `wifi_auth_mode_t`)_
